...
const DURATION_WINDOW_DURATION = 8;
...
const VideoTrimming = () => {
  const [selectedVideo, setSelectedVideo] = useState<VideoAsset | undefined>();
  ...
  const handlePressSelectVideoButton = async () => {
    ...
    if (!result.canceled) {
      setSelectedVideo({
        ...
        localFileName: getFileNameFromPath(result.assets[0].uri),
        // creationDate: new Date(result.assets[0]. || 0),
      });
    }
  };
  ...
  const handleVideoLoad = (videoAssetLoaded: any) => {
    ...
    setFrames(
      Array(numberOfFrames).fill({
        status: FRAME_STATUS.LOADING.name,
      })
    );
    ...
    FFmpegWrapper.getFrames(
      ...
      numberOfFrames,
      (filePath: string) => {
        const _framesURI: string[] = [];
        for (let i = 0; i < numberOfFrames; i++) {
          _framesURI.push(
            `${filePath.replace("%4d", String(i + 1).padStart(4, "0"))}`
          );
        }
        const _frames: Frame[] = _framesURI.map((_frameURI) => ({
          uri: _frameURI,
          status: FRAME_STATUS.READY.name,
        }));
        setFrames(_frames);
      }
    );
  ...
  const renderFrame = (frame: Frame, index: number) => {
    if (frame.status === FRAME_STATUS.LOADING.name) {
